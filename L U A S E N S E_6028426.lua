...
local nlgradient = require("neverlose/gradient")
...
ui.new_texture = function(tab, container, texture, sizex, sizey)
	local group = ui.create(tab, container)
	return group:texture(texture, vector(sizex, sizey))
end
...
return (function(z)
	...
	local linkcolor = ui.get_style()["Link Active"]
    z.prefix = {
        section = z.hex({55,55,55}) .. "[" .. z.hex({linkcolor.r, linkcolor.g, linkcolor.b}) .. "%s" .. z.hex({55,55,55}) .. "] " .. z.hex({255, 255, 255}) .. "- " .. z.hex({55,55,55}) .. "[" .. z.hex({169,169,169}) .. "%s" .. z.hex({55,55,55}) .. "] " .. z.hex({123,123,123}) .. "%s",
        x = z.hex({linkcolor.r, linkcolor.g, linkcolor.b}) .. "LS " .. z.hex({255, 255, 255}) .. "- " .. z.hex({123,123,123}) .. "%s"
    }
	z.infotab = function(type, args)
        return ui[type]("Info", unpack(args))
    end
    z.aatab = function(type, args)
        return ui[type]("Anti-Aim", unpack(args))
    end
	z.vistab = function(type, args)
        return ui[type]("Visuals", unpack(args))
    end
	z.misctab = function(type, args)
        return ui[type]("Misc", unpack(args))
    end
	local image_link = "https://i.imgur.com/u1GTHQ9.png"
	local image_draw = render.load_image(network.get(image_link))
    z.menu = {
		-- info tab
		ls1 = z.infotab("new_label", {"homepage", z.prefix.x:format("luasense")}),
		info_label = z.infotab("new_label", {"homepage", z.hex({123, 123, 123}) .. "Welcome " .. z.hex({linkcolor.r, linkcolor.g, linkcolor.b}) .. common.get_username() .. z.hex({255,160,160}) .. " [BETA]" }),
		ls_image = z.infotab("new_texture", {"Homepage", image_draw, 200, 200}),
		-- aa tab
		ls2 = z.aatab("new_label", {"anti-aimbot angles", z.prefix.x:format("luasense")}),
			select = z.aatab("new_combobox", {"anti-aimbot angles", "\n", "Builder", "Keybinds", "Config"}),
			["Builder"] = {
				state = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.x:format("State"), z.states}),
				builder = {},
				custom = {}
			},
			["Keybinds"] = {
				manualyaw = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.x:format("Manual yaw"), "None", "Left", "Right", "Forward"}),
			},
			
			["Config"] = {
				export = z.aatab("new_button", {"anti-aimbot angles", "Export", function()
					local cfg = {}
					for index, value in next, z.menu["Builder"] do
				...
				import = z.aatab("new_button", {"anti-aimbot angles", "Import", function()
					--pcall(function()
        ...
        -- visuals tab
		ls3 = z.vistab("new_label", {"watermark", z.prefix.x:format("luasense")}),
			watermark = z.vistab("new_combobox", {"watermark", z.prefix.x:format("Watermark"), {"Right", "Left"}}),
			luacolor = z.vistab("new_color_picker", {"watermark", z.prefix.x:format("Accent color"), 179, 255, 18,255}),
		
		ls4 = z.vistab("new_label", {"indicators", z.prefix.x:format("luasense")}),
			indicator = z.vistab("new_checkbox", {"indicators", z.prefix.x:format("Indicators")}),
			arrows = z.vistab("new_checkbox", {"indicators", z.prefix.x:format("Manual AA arrows")}),
			colorkey = z.vistab("new_color_picker", {"indicators", z.prefix.x:format("Active arrow color"), 179, 255, 18,255}),
			keycolor = z.vistab("new_color_picker", {"indicators", z.prefix.x:format("Inactive arrow color"), 55, 55, 55,255}),
			center = z.vistab("new_checkbox", {"indicators", z.prefix.x:format("Crosshair indicators")}),
			color = z.vistab("new_color_picker", {"indicators", z.prefix.x:format("Accent color"), 179, 255, 18,255}),

		-- misc tab
		ls5 = z.misctab("new_label", {"miscellaneous", z.prefix.x:format("luasense")}),
			fakelatency = z.misctab("new_checkbox", {"miscellaneous", z.prefix.x:format("Unlock fake latency")}),
			nadefix = z.misctab("new_checkbox", {"miscellaneous", z.prefix.x:format("Grenade throw fix")}),
			hideshot = z.misctab("new_checkbox", {"miscellaneous", z.prefix.x:format("Hideshot fix")}),
			animations = z.misctab("new_multiselect", {"miscellaneous", z.prefix.x:format("Animations"), "Static legs", "Moonwalk", "Leg fucker", "0 pitch on landing"}),
		ls6 = z.misctab("new_label", {"anti-aim miscellaneous", z.prefix.x:format("luasense")}),
			backstab = z.misctab("new_checkbox", {"anti-aim miscellaneous", z.prefix.x:format("Anti backstab")}),
			disabler = z.misctab("new_multiselect", {"anti-aim miscellaneous", z.prefix.x:format("FS Disablers"), {"Air", "Duck", "Slow"}})
			
    }
	z.tooltips = function()
		z.menu.backstab:tooltip("Makes the player face the enemy if he's close and has the knife in his hand")
		z.menu.nadefix:tooltip("Fixes the issue when you want to throw a grenade but you can't")
		z.menu.hideshot:tooltip("Makes hideshots less buggy")
		z.menu.fakelatency:tooltip("Makes the fake latency limit 150 instead of 100")
	end
	local function movecfg(arg)
        local x = string.find(ui.name(arg), "CT") and "CT" or "T"
        local xxx = string.find(ui.name(arg), "CT") and "T" or "CT"
        local tbl = z.menu["Builder"].builder[ui.get(z.menu["Builder"].state) .. " " .. xxx]
        for i, v in next, z.menu["Builder"].builder[ui.get(z.menu["Builder"].state) .. " " .. x] do
            if i ~= "button" then
	...
	for i, v in next, z.states do
        z.menu["Builder"].custom[v] = {
			desync = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Desync"), "Default ", "Custom"}),
			team = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Team"), "CT", "T"}),
            enable = z.aatab("new_multiselect", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Status"), "Enabled"}),
			yawteam = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Team"), {"CT", "T"}}),
			yawoption = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw mode"), {"Left right (FS: Yes)", "Left right (FS: No)", "Automatic (FS: No)"}}),
            tyawleft = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw left\nT"), -180, 180, -10}),
            tyawright = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw right\nT"), -180, 180, 10}),
			ctyawleft = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw left\nCT"), -180, 180, -10}),
            ctyawright = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw right\nCT"), -180, 180, 10}),
			yawauto = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Automatic"), {"Default", "Ways"}}),
			yawways =  z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Ways\n"), 2, 5, 5}),
			options = z.aatab("new_multiselect", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Defensive"), {"Pitch up", "Random yaw", "Offensive", "Always on"}})
        }
		ui["set_callback"](z.menu["Builder"].custom[v].yawways,function()
			logic = {}
		end)
    end
    z.cache = {}
    ...
    for i, v in next, z.cache do
        z.menu["Builder"].builder[v] = {
            enable = z.aatab("new_multiselect", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Status"), "Enabled"}),
            yawmode = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw options"), "Off", "Customize", "Customize + l & r", "Left and right"}),
            yaw = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw"), -180, 180, 0}),
            leftright = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Left right"), "Normal", "Simple", "Advanced"}),
            yawleft = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw left"), -100, 100, -10}),
            yawright = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw right"), -100, 100, 10}),
			yawtimer = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Yaw timer"), 1, 10, 1}),
            jitteryaw = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Jitter mode"), "Off", "Center", "Left right", "Random center", "Automatic small", "Automatic big"}),
            jitter = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Jitter"), -180, 180, 0}),
            jitterlr = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Left right"), -180, 180, 0}),
			jitterrandom = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Jitter\n\n\n"), 0, 90, 60}),
            jitterrnd = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Random"), 0, 30, 15}),
            fakeyaw = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Fake yaw"), "Default skeet", "Automatic fake", "Random skeet fake"}),
			bodymode = z.aatab("new_combobox", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Body yaw"), "Jitter", "Static", "Opposite", "Off"}),
            limityaw = z.aatab("new_slider", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Limit yaw"), 0, 60, 60}),
            options = z.aatab("new_multiselect", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Options"), {"Static freestanding", "Defensive", "Prioritize manual"}}),
            defensive = z.aatab("new_multiselect", {"anti-aimbot angles", z.prefix.section:format("AA", v, "Defensive"), {"Pitch up", "Random yaw", "Switch cycle", "Always on"}}),
            button = z.aatab("new_button", {"anti-aimbot angles", "Send to: " .. (string.find(v, "CT") and "T" or "CT"), movecfg}),
        }
    end
    z.hideshowetc = {
        handler = function()
                ...
                if type(value) == "table" then
                    for i, v in next, value do
                        if type(v) == "table" then
                            for ii, vv in next, v do
                                if type(vv) == "table" then
                                    for iii, vvv in next, vv do
										if (i == "custom") then
											if iii == "desync" then
												ui.set_visible(vvv, index == selected and ii == ui.get(z.menu["Builder"].state))
											end
											if iii == "team" then
												ui.set_visible(vvv, index == selected and ii == ui.get(z.menu["Builder"].state) and ui.get(z.menu["Builder"]["custom"][ui.get(z.menu["Builder"].state)].desync) == "Default ")
											end
											if iii == "enable" and ii == "Global" and ui.get(z.menu["Builder"].state) == "Global" then
												ui.set_visible(vvv, false)
											else
												if iii ~= "team" and iii ~= "desync" then
													local shortcut = z.menu["Builder"]["custom"][ui.get(z.menu["Builder"].state)]
													...
													ui.set_visible(vvv, toggle and index == selected and ii == ui.get(z.menu["Builder"].state) and ui.get(shortcut.desync) == "Custom")
												end
											end
										else
                                            local shortcut = z.menu["Builder"].builder[ui.get(z.menu["Builder"].state) .. " " .. ui.get(z.menu["Builder"]["custom"][ui.get(z.menu["Builder"].state)].team)]
											...
											if (toggle) then
												if (ui.get(z.menu["Builder"].state):find("Global")) then
													toggle = iii ~= "enable"
												else
													toggle = z.contains(ui.get(shortcut.enable), "Enabled") or iii == "enable"
												end
											end
											ui.set_visible(vvv, index == selected and ii == ui.get(z.menu["Builder"].state) .. " " .. ui.get(z.menu["Builder"]["custom"][ui.get(z.menu["Builder"].state)].team) and toggle and ui.get(z.menu["Builder"]["custom"][ui.get(z.menu["Builder"].state)].desync) == "Default ")
										end
                                    end
                                end
                            end
							...
							toggle = true
							if (i == "team") then
								toggle = ui.get(z.menu["Builder"].desync) == "Default "
							end
							if (i == "label" or i == "color" or i == "arrows" or i == "center") then
								toggle = ui.get(z.menu.indicator)
							end
			...
			if(ui.get(z.menu.indicator)) then
				ui.set_visible(z.menu.arrows, true)
				if(ui.get(z.menu.arrows)) then
					ui.set_visible(z.menu.keycolor, true)
					ui.set_visible(z.menu.colorkey, true)
				else
					ui.set_visible(z.menu.keycolor, false)
					ui.set_visible(z.menu.colorkey, false)
				end
				ui.set_visible(z.menu.center, true)
				if(ui.get(z.menu.center)) then
					ui.set_visible(z.menu.color, true)
				else
					ui.set_visible(z.menu.color, false)
				end
			else
				ui.set_visible(z.menu.arrows, false)
				ui.set_visible(z.menu.keycolor, false)
				ui.set_visible(z.menu.colorkey, false)
				ui.set_visible(z.menu.center, false)
				ui.set_visible(z.menu.color, false)
			end
        end,
        title = function()
            ...
            do
				...
				for idx = 1, #text_to_draw do
					...
					ui.set(z.menu.ls1, output)
					ui.set(z.menu.ls2, output)
					ui.set(z.menu.ls3, output)
					ui.set(z.menu.ls4, output)
					ui.set(z.menu.ls5, output)
					ui.set(z.menu.ls6, output)
				end
            end
        end,
        resetmenu = function()
    ...
    z.aa = {
        ...
        builder = function(arg)
            ...
            local state = z.getstate(myself, arg.in_jump  or air, duck, math.sqrt(xv*xv + yv*yv + zv*zv), fakelag, (ui.get(slow)))

			local cache = (ui.get(z.menu["Builder"].custom[state].desync) == "Default " and "builder" or string.lower(ui.get(z.menu["Builder"].custom[state].desync)))
			local fix = state
            ...
            if not z.contains(ui.get(z.menu["Builder"][cache][(cache == "builder" and fix or state)].enable), "Enabled") then
				cache = (ui.get(z.menu["Builder"].custom["Global"].desync) == "Default " and "builder" or string.lower(ui.get(z.menu["Builder"].custom["Global"].desync)))
			...
			local menutbl = z.menu["Builder"][cache][(cache == "builder" and fix or state)]
			local aatbl = z.items.aa
			ui.override(aatbl.enabled[1], true)
			ui.override(aatbl.roll[1], 0)
			--ui.set(aatbl.edge[1], false)
			...
			ui.override(aatbl.yaw[1], 0)
			ui.override(aatbl.pitch[1], "Down")
			ui.override(aatbl.base[1], script.cache.manual.aa == 0 and "at target" or "local view")
			
			if script.cache.manual.aa ~= 0 and z.contains(ui.get(menutbl.options), "Prioritize manual") then
				ui.override(aatbl.fs, false)
			end
			if z.contains(ui.get(z.menu.disabler), "Air") and (air or arg.in_jump) then
				ui.override(aatbl.fs, false)
			end
			if z.contains(ui.get(z.menu.disabler), "Duck") and duck then
				ui.override(aatbl.fs, false)
			end
			if z.contains(ui.get(z.menu.disabler), "Slow") and (ui.get(slow) and ui.get(key)) then
			    ui.override(aatbl.fs, false)
			end
			if ui.get(z.menu.nadefix) then
			...
			if ui.get(z.menu.hideshot) then
			...
			if ui.get(z.menu.backstab) then
        ...
        breakers = function(arg)
            ...
            local path = ui.get(z.menu.animations)
	...
	z.sidebar = function()
		...
		output = "L U A" .. output
		local linkcolor = ui.get_style()["Link Active"]
		local sidebar_gradient = nlgradient.text_animate("L U A", -3, {
			color(linkcolor.r, linkcolor.g, linkcolor.b), 
			color(linkcolor.r, linkcolor.g, linkcolor.b)
		})
		ui.sidebar(sidebar_gradient:get_animated_text() .. output:gsub("L U A", ""), ui.get_icon("kaaba"))
	end
	z.fakeping = function()
		if(ui.get(z.menu.fakelatency)) then
			cvar.sv_maxunlag:float(0.3)
		else
	...
	z.indicator = function()
		...
		output = "L U A" .. output
		local menutbl = z.menu
		local r,g,b,a = ui.get(menutbl.luacolor)
		renderer.text(width - (ui.get(menutbl.watermark) == "Right" and 85 or (width-55)), height - 555, r,g,b, 255, "c", 0, output)
		renderer.text(width - (ui.get(menutbl.watermark) == "Right" and 25 or (width-115)), height - 555, 255,160,160, 255, "c", 0, " [BETA]")
		-- local linkcolor = ui.get_style()["Link Active"]
		-- local sidebar_gradient = nlgradient.text_animate("L U A", -3, {
		-- 	color(linkcolor.r, linkcolor.g, linkcolor.b), 
		-- 	color(linkcolor.r, linkcolor.g, linkcolor.b)
		-- })
		-- ui.sidebar(sidebar_gradient:get_animated_text() .. output:gsub("L U A", ""), ui.get_icon("jedi"))
		...
		if ui.get(menutbl.manualyaw) == "None" then
            script.cache.manual["aa"] = 0
            script.cache.manual["tick"] = tick
        end
        if ui.get(menutbl.manualyaw) == "Left" then
            script.cache.manual["aa"] = -90
            script.cache.manual["tick"] = tick
        end
        if ui.get(menutbl.manualyaw) == "Right" then
            script.cache.manual["aa"] = 90
            script.cache.manual["tick"] = tick
        end
		if ui.get(menutbl.manualyaw) == "Forward" then
            script.cache.manual["aa"] = 180
            script.cache.manual["tick"] = tick
        end
		menutbl = z.menu
		...
		if ui.get(menutbl.center) then
			...
			if yaw_body > 0 then
				renderer.text( w, h + ind_height - 2.5, 255, 255, 255, 255, "c", nil, gradient(r2, g2, b2, a2, r1, g1, b1, a1, "luasense") )
			else
				renderer.text( w, h + ind_height - 2.5, 255, 255, 255, 255, "c", nil, gradient(r1, g1, b1, a1, r2, g2, b2, a2, "luasense") )
			end
			local dt_on = (ui.get(z.items.keys.dt[1]))
			local hs_on = (ui.get(z.items.keys.hs[1]))
    ...
    z.events = {
        ...
        paint_ui = { z.hideshowetc.title, z.hideshowetc.handler, z.tooltips, z.indicator, z.sidebar, z.fakeping },
    ...
    getstate = function(me, air, duck, speed, fl, slowcheck)
        ...
        if duck and not air and speed < 2 then state = "Duck" end
        if duck and not air and speed > 2 then state = "Duck moving" end
        if speed < 2 and not air and not duck then state = "Standing" end
        if speed > 2 and not air and not duck then state = "Moving" end
		if slowcheck and not air and not duck and speed > 2 then state = "Slow motion" end
        return state
    end,
})
